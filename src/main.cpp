...
#if defined(NDEBUG)
# error "Cryptocare cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x12bcca5766044829957b6a473a83d8b5a2135fcd24a8d97cb09a52a9e380e7a6");
...
const string strMessageMagic = "Cryptocare Signed Message:\n";
...
{
    // Cryptocare: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 47565 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 840960); // Cryptocare: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Cryptocare: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Cryptocare: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cryptocare: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cryptocare: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x67f46402bc2f694e8739de5a64727b809327b036699f5a6e5ce1b6e9197a33ee");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].nValue = 47565 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c68267bf0c8401825bca29ce1f8e0c1c708cb65a58274229aefebdb2403a28214b44837cfc869871c5be7a06cba3abd4c87175a2324a817aca38a86b1a385e39") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1542925836;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086861770;

        if (fTestNet)
        {
            block.nTime    = 1542925817;
            block.nNonce   = 386266375;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x84204c0c70ff2b6779142d37ac162810965eafb636061f39c6c0f25fe3292c32"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe3, 0xd4, 0xa1, 0xf8 }; 
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CryptocareMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CryptocareMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CryptocareMiner(CWallet *pwallet)
{
    printf("CryptocareMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cryptocare-miner");
    ...
    try { loop {
        ...
        printf("Running CryptocareMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CryptocareMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CryptocareMiner, pwallet));
}
